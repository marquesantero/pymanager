...
import { Package, Sun, Moon, Monitor, Plus, Folder, RefreshCcw, Trash2 } from "lucide-react";
...
export const Sidebar: React.FC<SidebarProps> = ({ 
  theme, setTheme, workspaces, activeWorkspace, setActiveWorkspace, addWorkspace, scanWorkspace, removeWorkspace 
}) => {
  return (
      ...
      <nav className="flex-1 px-4 py-6 space-y-6 overflow-y-auto text-[11px]">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5 pt-2">
            {workspaces.map(ws => (
              <div key={ws} onClick={() => setActiveWorkspace(ws)} className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer border transition-all ${activeWorkspace === ws ? "bg-blue-50 dark:bg-blue-900/20 text-blue-600 border-blue-200 dark:border-blue-800" : "bg-transparent border-transparent text-slate-500 hover:bg-slate-100"}`}>
                ...
                <div className="flex gap-1 shrink-0">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); removeWorkspace(ws); }} className="w-6 h-6 flex items-center justify-center rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-red-600 transition-all shadow-sm"><Trash2 size={10}/></button>
                </div>
              </div>
            ))}
          </div>
